...
function generateStudyPlan(studentData) {
    ...
    for (const [subject, score] of Object.entries(studentData)) {
        // Only add predefined recommendations for weak subjects
        if (score < 70) {  // For example, subject is weak if score < 70
            if (subject in predefinedRecommendations) {
                studyPlan[subject] = predefinedRecommendations[subject];
            }
        }
    }
...
try {
    ...
    const decodedData = Buffer.from(encodedInput, 'base64').toString('utf-8');
    const studentData = JSON.parse(decodedData);

    // Generate a study plan based on the student's weak subjects
    const studyPlan = generateStudyPlan(studentData);

    // Return the study plan as JSON
    console.log(JSON.stringify({ study_plan: studyPlan }));

} catch (e) {
    // Print any errors as JSON to ensure structured output
    const errorOutput = { error: e.toString() };

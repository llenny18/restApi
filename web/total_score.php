...
if ($result && $result->num_rows > 0) {
    ...
    $aud_score = $row['Fundamentals'] + $row['Risk_based'] + $row['Understanding'] + $row['Audit_Evidence'] + $row['Audit_Completion'] + $row['CIS'] + $row['Attestation'] + $row['Governance'] + $row['Risk_Response'];
    ...
    if ($stmt->execute()) {
        header("Location: algoResult.php");
    } else {

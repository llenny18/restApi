    ...
    <script>
        document.addEventListener('visibilitychange', function () {
            if (document.visibilityState === 'hidden') {
                // This triggers when the user switches tabs or minimizes the window
                alert("Warning: You switched away from the exam page. Please focus on the exam!");
            }
        });
    </script>
...
var timeLeft = 3 * 60 * 60; // 3 hours in seconds
var warningCount = 0; // To track the number of tab switches
...
function updateTimer() {
    ...
    // Notify at specific time intervals
    if (timeLeft === 9000) {
...
// Tab-switch detection to apply penalties
document.addEventListener("visibilitychange", function () {
    if (document.visibilityState === "hidden") {
        warningCount++;
        timeLeft -= 600; // Deduct 10 minutes (600 seconds)

        if (timeLeft < 0) timeLeft = 0; // Ensure timer doesn't go below 0

        alert(`You switched tabs! 10 minutes have been deducted from your time. Remaining time: ${Math.floor(timeLeft / 3600)}h ${Math.floor((timeLeft % 3600) / 60)}m.`);

        // Optionally, end the exam if the user switches tabs too many times
        if (warningCount >= 5) {
            alert("You have switched tabs too many times. Your exam will be submitted now.");
            document.getElementById("examForm").submit();
        }
    }
});
        ...
        <div id="question-container">
            ...
            foreach ($examTopics as $topicTable => $topicDisplayName) {
                if (isset($shuffledQuestions[$topicTable])) {
                    echo "<h3>" . htmlspecialchars($topicDisplayName, ENT_QUOTES, 'UTF-8') . " Questions</h3>";  // Display topic title
                    ...
                    foreach ($shuffledQuestions[$topicTable] as $scenario => $questions) {
                        ...
                        if ($scenario !== 'no_scenario') {
                            $scenarioText = preg_replace_callback(
                                '/\[Image:\s*(.+?)\]/i',
                                function ($matches) {
                                    $imagePath = htmlspecialchars($matches[1], ENT_QUOTES, 'UTF-8');
                        ...
                        foreach ($questions as $question) {
                            $questionId = htmlspecialchars($question['question_id'], ENT_QUOTES, 'UTF-8'); // Use a unique identifier for each question

                            echo '<div class="question-box">';

                            // Display question text with a proper question number
                            echo "<p><strong>Question $questionNumber:</strong> " . htmlspecialchars($question['question'], ENT_QUOTES, 'UTF-8') . "</p>";

                            // Display image (if available)
                            if (!empty($question['image'])) {
                                echo "<img src='" . htmlspecialchars($question['image'], ENT_QUOTES, 'UTF-8') . "' alt='Question Image' style='max-width: 100%; margin: 10px 0;'><br>";
                            }
                            ...
                            $correctAnswer = htmlspecialchars($question['answer'], ENT_QUOTES, 'UTF-8');  // Correct answer (A, B, C, D)
                            ...
                            foreach ($optionKeys as $key) {
                                $value = htmlspecialchars($options[$key], ENT_QUOTES, 'UTF-8'); // Get the answer text from the shuffled key
                                echo "<label>
